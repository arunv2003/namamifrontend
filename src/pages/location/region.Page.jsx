...
import RegionTable from '../../views/locations/region.Table';
import { locationRoute } from '../../routes/location/location.route';
import RegionFormModal from '../../components/dilogs/locations/region.modol';
...
export default function RegionPage() {
  ...
  const fetchRegions = useCallback(async () => {
    setLoading(true);
    try {
      const res = await locationRoute.getAllRegions({
        page: page + 1,
        limit: rowsPerPage,
      ...
      if (res && (res.success || res.statusCode === 200) && res.data) {
        const list = res.data.regions || (Array.isArray(res.data) ? res.data : []);
        setRegions(list);
  ...
  const handleEditClick = (rg) => {
    setSelectedRegion(rg);
    setFormModalOpen(true);
  };

  const handleDeleteClick = (rg) => {
    setRegionToDelete(rg);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const identifier = regionToDelete.slug || regionToDelete.id;
      const res = await locationRoute.deleteRegion(identifier);
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div>
              ...
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Manage regions within states, status settings, and state assignments.
              </p>
            </div>
          </div>
