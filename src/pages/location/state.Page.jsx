...
import StateTable from '../../views/locations/state.Table';
import { locationRoute } from '../../routes/location/location.route';
import StateFormModal from '../../components/dilogs/locations/state.modol';
...
export default function StatePage() {
  ...
  const fetchStates = useCallback(async () => {
    setLoading(true);
    try {
      const res = await locationRoute.getAllStates({
        page: page + 1,
        limit: rowsPerPage,
      ...
      if (res && (res.success || res.statusCode === 200) && res.data) {
        const list = res.data.states || (Array.isArray(res.data) ? res.data : []);
        setStates(list);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const identifier = stateToDelete.slug || stateToDelete.id;
      const res = await locationRoute.deleteState(identifier);
    ...
    >
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div>
              ...
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Manage state regions, status configurations, and coverage details.
              </p>
            </div>
          </div>
      ...
      {deleteModalOpen && (
        ...
        >
          ...
          <DialogContent>
            Are you sure you want to delete state <strong>{stateToDelete?.name}</strong>? This action cannot be undone.
          </DialogContent>
